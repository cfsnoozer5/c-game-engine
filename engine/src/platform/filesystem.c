...
b8 file_open(const char* path, file_modes mode, b8 binary, file_handle* out_handle) {
...
b8 file_close(file_handle* handle) {
    if (handle->handle) {
...
b8 file_read_line(file_handle* handle, char** line_buf) {
    if (handle->handle) {
        char buffer[32000]; // Bad practice, but no heap allocate
...
b8 file_write_line(file_handle* handle, const char* text) {
    if (handle->handle) {
...
b8 file_read(file_handle* handle, u64 data_size, void* out_data, u64* out_bytes_read) {
    if (handle->handle) {
...
b8 file_read_all_bytes(file_handle* handle, u8** out_bytes, u64* out_bytes_read){
    if (handle->handle) {
        // File size
...
b8 file_write(file_handle* handle, u64 data_size, const void* data, u64* out_bytes_written) {
    if (handle->handle) {
