...
// Pipelines are immutable! There is a little dynamic state, but say flip from wireframe to fill cannot be done
// New pipeline needs to be created for that
...
b8 vulkan_graphics_pipeline_create(
    ...
    czero_memory(&color_blend_attachment_state, sizeof(VkPipelineColorBlendAttachmentState));
    ...
    // Descriptor set layouts (used for uniforms)
    ...
    pipeline_create_info.subpass = 0; // Only primary right now
    ...
    VkResult result = vkCreateGraphicsPipelines(
        ...
        CDEBUG("Graphics pipeline created!");
        return true;
    }

    CERROR("vkCreateGraphicsPipelines failed with %s.", vulkan_result_string(result, true));
    return false;
}
...
void vulkan_pipeline_destroy(vulkan_context* ctx, vulkan_pipeline* pipeline) {
    if (pipeline) {
        if (pipeline->handle) {
            vkDestroyPipeline(ctx->device.logical_device, pipeline->handle, ctx->allocator);
            pipeline->handle = 0;
        }

        if (pipeline->pipeline_layout) {
            vkDestroyPipelineLayout(ctx->device.logical_device, pipeline->pipeline_layout, ctx->allocator);
