...
// Not really a free more of a release to pool
void vulkan_command_buffer_free(
    ...
    vkFreeCommandBuffers(
        context->device.logical_device,
        pool,
        1, // Will need to change if more are created
...
void vulkan_command_buffer_begin(
    vulkan_command_buffer* command_buffer,
    b8 is_single_use,
    b8 is_renderpass_contiue,
    b8 is_simultaneous_use) {
    
    VkCommandBufferBeginInfo begin_info = {VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO};
    begin_info.flags = 0;
    if (is_single_use) {
        begin_info.flags |= VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT; // Command Buffer only ever submitted once, oneshot
    }
    if (is_renderpass_contiue) {
        begin_info.flags |= VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT; // Secondary command buffer only
    }
    if (is_simultaneous_use) {
        begin_info.flags |= VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT; // Simultaneous use can be resubmitted to a queu while it's a pending state, recordered into multiple queues
    }
