...
b8 vulkan_object_shader_create(vulkan_context* context, vulkan_object_shader* out_shader) {
    ...
    // Attributes - Kind of hardcoded rn will need to be flexible
    u32 offset = 0;
    const i32 attribute_count = 1;
    VkVertexInputAttributeDescription attribute_descriptions[attribute_count];
    // Position
    VkFormat formats[attribute_count] = {
        VK_FORMAT_R32G32B32_SFLOAT  // vec3 32-bit floats
    };
    ...
    for (u32 i = 0; i < attribute_count; i++) {
        attribute_descriptions[i].binding = 0;   // Binding index
        attribute_descriptions[i].location = i;  // attrib location in shader
    ...
    if (!vulkan_graphics_pipeline_create(
            ...
            false,
            &out_shader->pipeline)) {
        CERROR("Failed to load graphics pipeline for object shader.")
        return false;
    }
    ...
    if (!vulkan_buffer_create(
            ...
            VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,                                             // Write only uniform Buffer
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,  // Host visible so we can uplaod directly
            true,
            &out_shader->global_uniform_buffer)) {
        CERROR("Vulkan uniform buffer creation failed for object shader.");
        return false;
    }
...
// Needs to be run within renderpass since cmdbuffer is utilized
void vulkan_object_shader_update_global_state(vulkan_context* context, struct vulkan_object_shader* shader) {
    ...
    if (!shader->descriptor_updated[image_index]) {
        ...
        // Copy Global Uniform Object to Buffer
    ...
    shader->descriptor_updated[image_index] = false;
}