...
typedef struct vulkan_physical_queue_family_info {
    u32 graphics_family_index;
    u32 present_family_index;
    u32 compute_family_index;
    u32 transfer_family_index;
} vulkan_physical_queue_family_info;
...
b8 physical_device_meets_requirements(
    ...
    vulkan_physical_queue_family_info* out_queue_family_info,
...
b8 vulkan_device_create(vulkan_context* context) {
    ...
    b8 present_share_graphics_queue = context->device.graphics_queue_index == context->device.present_queue_index;
    ...
    if (available_extension_count != 0) {
        ...
        for (u32 i = 0; i < available_extension_count; ++i) {
            if (strings_equal(available_extensions[i].extensionName, "VK_KHR_portability_subset")) {
                CINFO("Adding required extension 'VK_KHR_portability_subset'.");
    ...
    pool_create_info.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT; // Allows any buffer allocated from the pool to be individually reset
...
b8 select_physical_device(vulkan_context* context) {
    ...
    for (u32 i = 0; i < physical_device_count; i++) {
        ...
        vulkan_physical_queue_family_info queue_info = {};
...
b8 physical_device_meets_requirements(
    ...
    const VkPhysicalDeviceFeatures* features,
    const vulkan_physical_device_requirements* requirements,
    vulkan_physical_queue_family_info* out_queue_info,
    vulkan_swapchain_support_info* out_swapchain_support) {
    // Evalute device properties to determine
    ...
    // Look at each queu and see what queues it supports
    ...
    if (
        ...
        if (out_swapchain_support->format_count < 1 || out_swapchain_support->present_mode_count < 1) {
            ...
            CINFO("Required swapchain support no present, skipping device.");
            return false;
        }
