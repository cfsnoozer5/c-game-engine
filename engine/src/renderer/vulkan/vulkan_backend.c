...
b8 vulkan_renderer_backend_initialize(struct renderer_backend* backend, const char* application_name) {
    ...
    app_info.pEngineName = "Cai Engine";
    ...
    platform_get_required_extension_names(&required_extensions);       // Platform-specfic extenion(s)
#if defined(_DEBUG)
...
// Validation only needed in DEBUG mode
#if defined(_DEBUG)
...
// Attach Debugger
    ...
    // because the intial state should be 0 and will be 0 when not in use. Actual fences are not
    // owned by the list
...
b8 vulkan_renderer_backend_begin_frame(struct renderer_backend* backend, f32 delta_time) {
    ...
    VkDeviceSize offests[1] = {0};
    vkCmdBindVertexBuffers(command_buffer->handle, 0, 1, &context.object_vertex_buffer.handle, (VkDeviceSize*)offests);
...
b8 vulkan_renderer_backend_end_frame(struct renderer_backend* backend, f32 delta_time) {
    ...
    // Wait semaphore(s) ensure that the operation cannot begin until the image is available
    submit_info.waitSemaphoreCount = 1;
    submit_info.pWaitSemaphores = &context.image_available_semaphores[context.current_frame];

    // Each sempahore waits on the corresponding pipeling stage to complete
...
b8 create_buffers(vulkan_context* context) {
    ...
    if (!vulkan_buffer_create(
        ...
        CERROR("Error creating vertex buffer.");
        return false;
    }
