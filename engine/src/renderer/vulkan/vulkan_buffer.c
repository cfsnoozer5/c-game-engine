...
b8 vulkan_buffer_create(
    ...
    buffer_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;  // NOTE: Only used in one queue. We aren't using multiple right now
...
b8 vulkan_buffer_resize(
    ...
    if (result != VK_SUCCESS) {
        CERROR("Unable to resize vulkan buffer because the required memory allocation failed. Error: %i", result);
        return false;
    }
