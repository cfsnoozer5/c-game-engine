...
// Swapchains are immutable, if window is resized we will have to recreate
void vulkan_swapchain_recreate(
    vulkan_context* context,
    u32 width,
    u32 height,
    vulkan_swapchain* out_swapchain) {
    // Destroy the old and create a new chain
...
void vulkan_swapchain_present(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
        ...
        CFATAL("Failed to present swap chain image!");
    }
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain) {
    ...
    swapchain->max_frames_in_flight = 2; // Triple buffering, drawing 3 total frames, one presenting and 2 in background
    ...
    VkPresentModeKHR present_mode = VK_PRESENT_MODE_FIFO_KHR; // Always display all images first in first out
    for (u32 i = 0; i < context->device.swapchain_support.present_mode_count; i++) {
        ...
        // Use most current Image Available throw out old ones
    ...
    if (context->device.graphics_queue_index != context->device.present_queue_index) {
        ...
        // Going to use the same queue if graphis and present are the same
        swapchain_create_info.imageSharingMode = VK_SHARING_MODE_EXCLUSIVE;
        swapchain_create_info.queueFamilyIndexCount = 0;
        swapchain_create_info.pQueueFamilyIndices = 0;
    }

    swapchain_create_info.preTransform = context->device.swapchain_support.capabilities.currentTransform; // Uses device current transform, if device needs flipped this needs to  be updated
    ...
    vulkan_image_create(
        ...
        CINFO("Swapchain created sucesfully.");
}
...
void destroy(vulkan_context* context, vulkan_swapchain* swapchain) {
    ...
    // Images are owned by the swapchain itself so destroyed by the swapchain
    for (u32 i = 0; i < swapchain->image_count; i++) {
